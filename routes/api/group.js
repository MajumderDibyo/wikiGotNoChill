...
connection.connect(function(err){
 ...
 router.post("/create", (req, res) => {
   ...
   connection.query("INSERT INTO group_name(creator_id, group_name, tag) values(?, ?, ?)", [createrId, name, tag],function(error, results, fields){
     if (error) {
...
router.post("/like/:post_id", (req, res) =>{
  ...
  connection.query("SELECT likes_count from posts where post_id = ?",[post_id], function(error, results, fields){
    if(error){
            console.log("error ocurred",error);
               res.send({
                    "code":400,
           ...
           likes ++;
           connection.query("UPDATE posts set likes_count = ? where post_id = ?",[likes, post_id], function(error, results, fields){
             if(error){
               console.log("error occoured", error);
               res.send({
                 "code":400,
...
router.post("/fetchPosts/:gid", (req, res) =>{
  var group_id = req.params.gid;
  var objs = [];
  connection.query("SELECT post_id, user_id from group_posts where group_id = ?",[group_id], function(error, results, fields){
    if(error){
      console.log("error occoured", error);
      res.send({
        "code":400,
      ...
      for(var i = 0; i < results.length; i++){
        ...
        connection.query("SELECT post_content, likes_count from posts where post_id = ?",[post_id], function(error, result, fields) {
          if(error){
            console.log("error occoured", error);
            res.send({
              "code":400,
              ...
              objs.push({userid: user_id, postid: post_id, content: result[0].post_content, likes: result[0].likes_count});
        }
      });
      }
      res.end(JSON.stringify(objs));
...
router.post("/groupSuggestions", (req, res) =>{
  var user_id = sess.user_id;
  connection.query("SELECT tag from interests where type = 1 and user_id = ?",[user_id], function(error, results, fields){
    if(error){
      console.log("error occoured", error);
      res.send({
        "code":400,
      ...
      for(var i = 0; i < results.length; i++){
        ...
        var objs = [];
        connection.query("SELECT group_id, group_name from group_name where tag = ?", [tag], function(error, results, fields){
          if(error){
            console.log("error occoured", error);
            res.send({
              "code":400,
